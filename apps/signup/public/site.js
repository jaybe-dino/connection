...
form.addEventListener('submit',async e=>{e.preventDefault();if(busy)return;if(!learning||form.site_url.value.trim()!==learnedUrl){show('#submitStatus','브랜드 홈페이지 분석을 완료해 주세요.',true);return;}busy=true;document.querySelector('#submitButton').disabled=true;show('#submitStatus','가입 신청을 저장하고 있습니다…');
 ...
 const result=await request('/applications',{slug:form.slug.value.trim(),name:form.elements.name.value.trim(),biz_no:form.biz_no.value,category:form.category.value,countries:[form.country.value],contact:form.contact.value.trim(),site_url:learnedUrl,learning_id:learning.learningId,answers,plan:'per_signup'});
