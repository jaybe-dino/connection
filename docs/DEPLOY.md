...
| 프로젝트 이름 | Root Directory | 용도 |
|---|---|---|
| connection-creator | `apps/creator-app` | 크리에이터 PWA |
| connection-console | `apps/console` | 브랜드 콘솔 |
| connection-signup | `apps/signup` | 브랜드 가입 위저드 |

각 프로젝트 공통:
- **Framework Preset**: `Vite` (Next.js 아님!)
- **Root Directory**: 위 표대로 (Edit 눌러 지정)
- Build/Install 명령은 기본값 그대로 (pnpm 워크스페이스 자동 인식)
- **Environment Variables**: `VITE_API_URL` = 1-6에서 복사한 Railway 주소
  (예: `https://xxx.up.railway.app` — 끝에 `/` 없이)
- Deploy
