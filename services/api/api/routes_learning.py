...
def learn(body:LearnIn,request:Request):
    ...
    try:
        ...
        raise HTTPException(422 if isinstance(e,brand_learning.LearningError) else 502,str(e) if isinstance(e,brand_learning.LearningError) else '분석 서비스가 응답하지 않았습니다. 잠시 후 다시 시도하세요.')
...
def save_profile(brand:str,body:ProfileIn,authorization:str=Header(default='')):
    ...
    with connect() as c:
        ...
        row=c.execute("SELECT fields FROM brand_learning WHERE learning_id=%s AND state='ready'",(body.learning_id,)).fetchone()
        if not row:raise HTTPException(400,'완료된 학습 결과가 필요합니다')
        ...
        for k,v in body.answers.items():
            if k in {'brand_one_liner','hero_product','ideal_creator','banned_words','sample_criteria','voice'} and v.strip():fields[k]={'value':v[:2000],'source':'brand_confirmed','confirmed':True}
